...
function beginExecution() {
    ...
    $(".search-txt").bind('input', function () {
        deselectObject();
        var searchtext = $('.search-txt').val().trim();
        //var searchtext = 'Hiran Silva';
        setTimeout(function(){
             var obj = graph.data[searchtext];
            if (obj) {
                selectObject(obj);
            }       
        }, 500);
...
function drawGraph() {
    ...
    for (var name in graph.data) {
        ...
        for (var depIndex in obj.depends) {
            var dependentOnBy = graph.data[obj.depends[depIndex]];
            if(excluded.indexOf(dependentOnBy.type) === -1) {
                var link = {
                    source : obj,
                    target : dependentOnBy
                };
    ...
    graph.legend = graph.svg.append('g')
      ...
      .enter().append('g').on('click', function(d) {
            if (d.type === 'All') {
                for (var name in graph.categories) {
                    var cat = graph.categories[name];
                    if (excluded.indexOf(cat.type) === -1) {
                        excluded.push(cat.type);
                    }
                }
            } else {
                if (excluded.indexOf(d.type) > -1){                
                    excluded.splice(excluded.indexOf(d.type), 1);
                } else {
                    excluded.push(d.type);
                }           
            }
    ...
    graph.legend.append('text')
        ...
        .text(function(d) {
            var count = (d.key === 'All:')? '' : ' (' + graph.categories[d.key].count + ')';
            return d.typeName + (d.group ? ': ' + d.group : '') +  count;
        });
...
function tick(e) {
    ...
    graph.line
        .attr('x1', function(d) {
            return isNaN(d.source.x)? randomX(): d.source.x;
        })
        .attr('y1', function(d) {
            return isNaN(d.source.y)? randomY(): d.source.y;
        })
        .each(function(d) {
            var x    = d.target.x,
                y    = d.target.y,
            ...
            d3.select(this)
                .attr('x2', isNaN(x)? randomX(): x)
                .attr('y2', isNaN(y)? randomY(): y);
        });

    graph.node
        .attr('transform', function(d) {
            d.x = isNaN(d.x)? randomX(): d.x;
            d.y = isNaN(d.y)? randomX(): d.y;
...
function randomX() {
    //return Math.floor((Math.random() * $('#graph-container').width()));
    return 0;
}

function randomY() {
    //return Math.floor((Math.random() * $('#graph-container').height()));
    return 0;
}
